...
export async function POST(req: Request) {
  ...
  // P0.15/H-1 — advertising is a plan feature; UI hiding is not authz.
  try {
    await requirePlanFeature(user.id, "advertising");
  } catch (e) {
    const status = e instanceof AuthError ? e.status : 403;
    const msg = e instanceof AuthError ? e.message : "امکان تبلیغات در پلن فعلی شما فعال نیست.";
  ...
  try {
    const ad = await createAdDraft({
      ...
      // H-1 (2-c): ad campaigns carry no financial flow in this stage —
      // the displayed price is bookkeeping ONLY and must never be
      // client-authoritative. Server-side pricing is enforced (0); a
      // future monetization flow must derive the price server-side.
      priceRials: 0,
      ip,
    });
